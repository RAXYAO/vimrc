...
" Environment {
    ...
    " Windows Compatible {
        ...
        "if WINDOWS()
          "set runtimepath=$HOME/.vim,$HOME/.vimfiles,$VIMRUNTIME,$VIM/.vimfiles/after,$HOME/.vim/after
        "endif
    " }
    " Python setup {
        " let $PYTHONHOME = '/opt/anaconda2/bin'
        " let $PYTHON_HOST_PROG = '/usr/bin/python'
        " let $PYTHON3_HOST_PROG = '/usr/bin/python3'
        " let $PYTHON_HOST_PROG = '/opt/anaconda2/bin/python2.7'j
        " let $PYTHON3_HOST_PROG = '/usr/bin/python3'
...
" Plugins {
    ...
    " PIV {
        if isdirectory(expand("$HOME/.vimfiles/plugged/PIV"))
    ...
    " Startify {
    if isdirectory(expand("$VIM/vimfiles/plugged/vim-startify"))
    ...
    " SnipMate {
        ...
        let g:snips_author = 'Rax Yao <raxyao@gmail.com>'
    " }
    ...
    " NerdTree {
        if isdirectory(expand("$HOME/.vimfiles/plugged/nerdtree"))
            ...
            let NERDTreeIgnore=['\.py[cd]$', '\~$', '\.swo$', '\.swp$', '^\.git$', '^\.hg$', '^\.svn$', '\.bzr$','\.class$']
    ...
    " Tabularize {
        if isdirectory(expand("$HOME/.vimfiles/plugged/tabular"))
    ...
    " Session List {
        ...
        if isdirectory(expand("$HOME/.vimfiles/plugged/sessionman.vim/"))
    ...
    " PyMode {
        ...
        if isdirectory(expand("$HOME/.vimfiles/plugged/python-mode"))
    ...
    " Rainbow {
        if isdirectory(expand("$HOME/.vimfiles/plugged/rainbow/"))
    ...
    " }
    " coc {
        if count(g:spf13_plug_groups, 'coc')
    ...
    " deoplete {
        if count(g:spf13_plug_groups, 'deoplete')
            ...
            setlocal dictionary+=$HOME.'/OtherComponents/words.txt'
    ...
    " Snippets {
        if count(g:spf13_plug_groups, 'neocomplcache') ||
                    \ count(g:spf13_plug_groups, 'neocomplete')

            " Use honza's snippets.
            let g:neosnippet#snippets_directory='$HOME/.vimfiles/plugged/vim-snippets/snippets'
    ...
    " indent_guides {
        if isdirectory(expand("$HOME/.vimfiles/plugged/vim-indent-guides/"))
    ...
    " vim vimcmdline
        if isdirectory(expand("$HOME/.vimfiles/plugges/vimcmdline"))
            " vimcmdline mappings
...
" Functions {
    ...
    function! s:IsSpf13Fork()
        let s:is_fork = 0
        let s:fork_files = ["$HOME/.vimrc.fork", "~/.vimrc.before.fork", "~/.vimrc.plug.fork"]
    ...
    function! s:EditSpf13Config()
        ...
        call <SID>ExpandFilenameAndExecute("vsplit", "$HOME/.vimrc.plug")
        ...
        wincmd l
        call <SID>ExpandFilenameAndExecute("split", "$HOME/.vimrc.plug.local")
        ...
        if <SID>IsSpf13Fork()
            ...
            wincmd l
            call <SID>ExpandFilenameAndExecute("split", "$HOME/.vimrc.plug.fork")
        endif
